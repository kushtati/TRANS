...
router.post('/refresh', async (req: Request, res: Response) => {
  try {
    ...
    if (!storedToken) {
      ...
      log.warn('Refresh token reuse detected', { userId: decoded.userId });
      return res.status(401).json({
        success: false,
    ...
    const tokens = generateTokens(storedToken.user.id, storedToken.user.role, storedToken.user.companyId);
...
router.post('/forgot-password', async (req: Request, res: Response) => {
  try {
    ...
    // Generate and store reset code (reuse verification code field)
    const code = generateVerificationCode();
    ...
    // Send email
    await sendVerificationEmail(user.email, code, user.company.name);

    log.audit('Password reset requested', { userId: user.id });
