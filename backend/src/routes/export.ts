...
import { generateInvoicePDF, buildInvoiceData } from '../services/invoice-pdf.service.js';
...
// ============================================
// GET /api/export/shipment/:id/facture
// Generate client invoice PDF (EMERGENCE-style template)
// ============================================
...
router.get('/shipment/:id/facture', async (req: Request, res: Response) => {
  try {
    ...
    // Calculate montantPayeClient from provisions
    const provisions = shipment.expenses.filter(e => e.type === 'PROVISION');
    const montantPayeClient = provisions.reduce((s, e) => s + e.amount, 0);

    // Generate invoice number: FAC-{trackingNumber} or use query param
    const invoiceNumber = (req.query.invoiceNumber as string) || shipment.trackingNumber.replace(/[^\d]/g, '') || shipment.trackingNumber;

    // Build data and generate PDF
    const invoiceData = buildInvoiceData(shipment, company, invoiceNumber, montantPayeClient);
